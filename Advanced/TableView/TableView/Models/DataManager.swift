...
//  TableView
//
//  Created by Kaala on 2023/05/17.
//
...
struct DataManager {
    ...
    private mutating func makeMovieData(){
        self.movieDataArray = [
    ...
    mutating func addMovieData()->Bool{
        let previousLength = movieDataArray.count
//        let index = Int.random(in: 0..<movieDataArray.count)
//        print("data index : \(index)")
        let data = movieDataArray[Int.random(in: 0..<movieDataArray.count)]
        movieDataArray.append(data)
        return movieDataArray.count > previousLength
    }
